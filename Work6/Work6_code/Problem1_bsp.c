...
void Display_Number(int x){		//Display function for integer numbers.
	clear_SSD();
	/*Check the number of digits and whether it is positive or not.*/
	if (ndigit(x) == 1 && x >=0){
		setSSD(x % 10);
		TurnOn_Digit(4);
		clear_Digit(4);
	...
	/*Minus symbol must be moved depending on the number of digits.*/
	else if (x < 0){
		GPIOA->ODR &= ~(1U << 7);	//PA7	G for minus symbol
		if (ndigit(x) == 1){
			TurnOn_Digit(3);
			clear_Digit(3);
			setSSD(abs(x) % 10);
